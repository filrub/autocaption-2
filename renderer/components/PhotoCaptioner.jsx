...
const PhotoCaptioner = memo(function PhotoCaptioner({
  ...
  const recognizeFaces = async () => {
    ...
    if (processingRef.current) {
      console.log(`Skipping recognition for ${photo.filename} - already processing`)
      return
    }
    ...
    try {
      ...
      let facesInPhoto = photo.faces?.length > 0 
        ? photo.faces 
        : await window.electronAPI.insightFaceDetection(photo, insightFaceServer)
      ...
      onPhotoUpdate(photoIndex, { faces: recognizedFaces })
    } catch (error) {
