...
if (process.contextIsolated) {
  try {
    ...
    contextBridge.exposeInMainWorld('electronAPI', {
      ...
      listImages: (targetFolder) => ipcRenderer.invoke('list-images', targetFolder),
      onImageLoadingProgress: (callback) => {
        ipcRenderer.on('image-loading-progress', (event, data) => callback(data))
      ...
      writeIptc: (targetFolder, photo, caption) =>
        ipcRenderer.invoke('writeIptc', targetFolder, photo, caption),
      handleEnrollPerson: (person) => ipcRenderer.invoke('handleEnrollPerson', person),
      ...
      checkRecognitionService: () => ipcRenderer.invoke('check-recognition-service'),
      startRecognitionServer: () => ipcRenderer.invoke('startRecognitionServer')
    })
  } catch (error) {
    console.error(error)
  }
